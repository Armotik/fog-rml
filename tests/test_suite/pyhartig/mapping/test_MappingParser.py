...
from pyhartig.mapping.MappingParser import FNML, FNO, RDF, RML, RR, MappingParser
...
def test_mapping_parser_graph_and_join_helper_methods(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("pyhartig.mapping.MappingParser.SourceFactory.create_source_operator", lambda **kwargs: object())
...
def test_mapping_parser_join_and_parent_resolution_helpers_cover_internal_branches(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("pyhartig.mapping.MappingParser.EquiJoinOperator", lambda *args, **kwargs: object())
