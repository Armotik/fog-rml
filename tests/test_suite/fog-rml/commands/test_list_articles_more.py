...
def test_fetch_all_py_prefers_root_env_over_data_env(tmp_path):
    import importlib.util

    script = Path(__file__).resolve().parents[3] / "use_cases" / "list_articles" / "fetch_all_py.py"
    spec = importlib.util.spec_from_file_location("fetch_all_py_test", script)
    ...
    (data_dir / ".env").write_text(
        ...
        "SERPAPI_KEY=from-data\n",
        encoding="utf-8",
    )
    (root_dir / ".env").write_text(
        "OPENALEX_AUTHOR='Olaf Hartig'\n",
        encoding="utf-8",
    )

    monkeypatched_base = root_dir
    module.BASE = monkeypatched_base
    env = module._load_env_files()

    assert env["OPENALEX_AUTHOR"] == "Olaf Hartig"
    assert env["SERPAPI_KEY"] == "from-data"
