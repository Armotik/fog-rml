...
from fog_rml.namespaces import XSD_STRING
...
class NTriplesSerializer:
    ...
    def serialize(self, row: MappingTuple):
        """
        ...
        # Return structured info: (line, key, is_quad)
    ...
    def _format_term(self, term, allowed_types) -> str:
        ...
        if isinstance(term, IRI):
            # Ensure IRI is percent-encoded for path/query/fragment components
            try:
                parts = urlsplit(term.value)
                # Keep common path/query characters (including ';') unescaped to match
                # expected test-case IRIs that may include semicolons.
