...
class MysqlSourceOperator(CsvSourceOperator):
	...
	def _normalize_dsn(dsn: str) -> str:
		...
		if dsn_str == "CONNECTIONDSN":
			env_dsn = os.getenv("FOG_RML_MYSQL_DSN") or os.getenv("FOG_RML_MYSQL_DSN") or os.getenv("MYSQL_DSN")
	...
	def _load_rows(
		...
		try:
			rows = cls._fetch_rows(pymysql, conn_kwargs, sql)
		except Exception:
	...
	def _import_pymysql():
		"""
		Imports the `pymysql` module required for MySQL access.
		:return: Imported `pymysql` module.
		"""
		try:
	...
	@staticmethod
	def _fetch_rows(pymysql, conn_kwargs: Dict[str, Any], sql: str) -> list:
		"""
		Executes a SQL query through pymysql and returns row dictionaries.
		:param pymysql: Imported `pymysql` module.
		:param conn_kwargs: pymysql connection keyword arguments.
