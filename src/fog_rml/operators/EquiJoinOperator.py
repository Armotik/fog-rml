...
class EquiJoinOperator(Operator):
    """
    EqJoin^J(râ‚, râ‚‚) : Operator Ã— Operator â†’ Operator

    Combines two mapping relations râ‚ = (Aâ‚, Iâ‚) and râ‚‚ = (Aâ‚‚, Iâ‚‚) based on
    a set of join conditions J âŠ† Aâ‚ Ã— Aâ‚‚.

    Preconditions:
    - Aâ‚ âˆ© Aâ‚‚ = âˆ… (attribute sets must be disjoint)
    - J is a set of pairs (aâ‚, aâ‚‚) where aâ‚ âˆˆ Aâ‚ and aâ‚‚ âˆˆ Aâ‚‚
    ...
    def __init__(
        ...
        :param r_1: The left child operator providing mapping relation râ‚ = (Aâ‚, Iâ‚)
        :param r_2: The right child operator providing mapping relation râ‚‚ = (Aâ‚‚, Iâ‚‚)
        :param left_join_attributes: List of attribute names from Aâ‚ to join on (left side of J pairs)
        :param right_join_attributes: List of attribute names from Aâ‚‚ to join on (right side of J pairs)
        ...
        # J = { (aâ‚, aâ‚‚) | aâ‚ âˆˆ A, aâ‚‚ âˆˆ B } - the join condition pairs
    ...
    def execute(self) -> Iterable[MappingTuple]:
        ...
        For each pair of tuples (tâ‚, tâ‚‚) from the two relations, if all join
    ...
    def _satisfies_join_condition(self, t1: MappingTuple, t2: MappingTuple) -> bool:
        ...
        âˆ€(aâ‚, aâ‚‚) âˆˆ J : tâ‚(aâ‚) = tâ‚‚(aâ‚‚)

        :param t1: Tuple from the left relation (tâ‚ âˆˆ Iâ‚)
        :param t2: Tuple from the right relation (tâ‚‚ âˆˆ Iâ‚‚)
