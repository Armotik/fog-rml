...
- Execute: the callable is invoked with evaluated positional arguments; its return value is inserted into the
    algebraic pipeline (preferably as a `fog-rml` RDF term).
...
- Runtime input to the Python callable: evaluated argument values (often `fog-rml.algebra.Terms.Literal`,
    `IRI`, or `BlankNode` objects). Callables should unwrap `.lexical_form` / `.value` as needed.
- Output from the callable: ideally a `fog-rml` RDF term (`IRI`, `Literal`, `BlankNode`) so the pipeline can
